    ...
    if (newlaserCloud) {
      ...
      if (stairEnable) {
        terrain_analysis::DetectTraversableTransitions(
          planarVoxelElev, planarPointElev, planarVoxelWidth, minBlockPointNum,
              ...
              if (disZ >= 0 && disZ < vehicleHeight &&
                ...
                terrainCloudElev->points[terrainCloudElevSize].intensity =
                    terrain_analysis::PointIntensity(
                      point.z, planarVoxelElev[cell], disZ,
                      planarTransitionCell[cell], planarVoxelElev,
                      planarSupportCell, planarVoxelWidth, cell, trav_params);
